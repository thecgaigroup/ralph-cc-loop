...
| Pattern | Risk | Action |
|---------|------|--------|
| Patch: `1.0.0 → 1.0.1` | Low | Auto-merge if CI passes |
| Minor: `1.0.0 → 1.1.0` | Low-Medium | Auto-merge if CI passes |
| Major: `1.0.0 → 2.0.0` | High | Review changelog, may need code changes |
...
```bash
# Approve
gh pr review NUMBER --repo owner/repo --approve --body "✅ Auto-approved: Patch/minor update, CI passes
...
gh pr comment NUMBER --repo owner/repo --body "⚠️ **Major version bump detected**
...
Identify by: PR body contains "🤖 Generated by Ralph"
...
**If review passes**:
```bash
gh pr review NUMBER --approve --body "✅ **Ralph PR Review**
...
### Auto-Merged (3)
- #12 ✅ bump lucide-react 0.469.0 → 0.562.0 (minor)
- #10 ✅ bump email-validator 2.1.0 → 2.3.0 (minor)
- #7 ✅ bump python-multipart 0.0.6 → 0.0.21 (patch)

### Approved, Pending Merge (1)
- #14 ✅ Fix Composer Audio Playback (Ralph PR)

### Flagged for Human Review (2)
- #5 ⚠️ bump next 15.5.9 → 16.1.1 (MAJOR)
- #1 ⚠️ bump python 3.12 → 3.14 (MAJOR)
